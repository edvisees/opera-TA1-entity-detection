...
def extract_ner(sent):
    ...
    for wid, word in enumerate(sent.words):
        if ners[wid][0] == 'B':
                ...
                named_ent = {'mention': sent.sub_string(wid, j), 'category': 'NAM', 'type': type, 
                'char_begin': char_begin, 'char_end': char_end, 'head_span': head_span, 'headword': sent.words[j-1].word}
                ### gazateer
